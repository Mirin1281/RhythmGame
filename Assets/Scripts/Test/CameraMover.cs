...
public class CameraMover : MonoBehaviour
{
    ...
    void MoveTo(CameraMoveSetting m, bool isInverse = false)
    {
        ...
        var startRotate = mainCamera.transform.eulerAngles;
            ...
            if(m.IsRotateClamp)
            {
                WhileYield(m.Time, t => 
                {
                    baseRot = Quaternion.Euler(
                        t.Ease(startRotate.x, GetNormalizedAngle(m.Rotate.x, startRotate.x - 180, startRotate.x + 180), m.Time, m.EaseType),
                        a * t.Ease(startRotate.y, GetNormalizedAngle(m.Rotate.y, startRotate.y - 180, startRotate.y + 180), m.Time, m.EaseType),
                        a * t.Ease(startRotate.z, GetNormalizedAngle(m.Rotate.z, startRotate.z - 180, startRotate.z + 180), m.Time, m.EaseType));
                });
            }
            else
            {
                ...
                {
                    baseRot = Quaternion.Euler(
                        t.Ease(startRotate.x, m.Rotate.x, m.Time, m.EaseType),
                        a * t.Ease(startRotate.y, m.Rotate.y, m.Time, m.EaseType),
                        a * t.Ease(startRotate.z, m.Rotate.z, m.Time, m.EaseType));
                });
            }
        }
    }
    ...
    {
        int a = isInverse ? -1 : 1;
        mainCamera.transform.localRotation = Quaternion.Euler(0f, 0f, a * strength);
        WhileYield(time, t => 
        {
            mainCamera.transform.localRotation = Quaternion.Euler(0f, 0f, a * t.Ease(strength, 0, time, easeType));
        });
    }
