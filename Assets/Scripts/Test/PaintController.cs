...
public class PaintController : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        var imagePos = ts.anchoredPosition;
        _disImagePos = new Vector2(imagePos.x - rect.width / 2f, imagePos.y - rect.height / 2f);
        _color = _randomColor ? Color.HSVToRGB(Random.Range(0, 1f), 1, 0.85f) : _lineColor;
        _isClear = true;
        _newImage.gameObject.SetActive(false);


        void SetTextureColor(Texture2D tex, Color32 color)
        {
