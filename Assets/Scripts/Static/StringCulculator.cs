    ...
    public static class Calc
    {
        ...
        class NodeTree : NodeBase
        {
            ...
            public float Calc(Func<string, List<float>, float> func)
            {
                return op switch
                {
                    Operator.Plus => CulcLPB(term1.Calc(func), term2.Calc(func), true), //return term1.Calc(func) + term2.Calc(func);
                    Operator.Minus => CulcLPB(term1.Calc(func), term2.Calc(func), false),
                    Operator.Multi => term1.Calc(func) * term2.Calc(func),
                    Operator.Divide => term1.Calc(func) / term2.Calc(func),
                    Operator.Mod => term1.Calc(func) % term2.Calc(func),
                    _ => 0,
                };


                static float CulcLPB(float a, float b, bool addOrExcept)
                {
                    if (a == 0)
                    {
                        if (b == 0)
